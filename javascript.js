...
//convrt undefined values to null
...
function undefinedToNull(input) {
  if (Array.isArray(input)) {
    ...
    // Handle objects: process each key-value pair recursively
    return Object.keys(input).reduce((result, key) => {
      result[key] =
        input[key] === undefined ? null : undefinedToNull(input[key]);
      return result;
    }, {});
  } else {
...
// Caret (^):
// Use for most dependencies, especially when you
// want minor updates and patches for bug fixes and non-breaking improvements.
// Tilde (~):
